...
simulation.prototype = {
    ...
    drawGrid: function(){
        ...
        var c = document.getElementById("canvas");
        var ctx = c.getContext('2d');
        for (var x = 0; x < c.width; x = x + this.GRIDSIZE){
            var line = this.GRID.VERTICALS[x];
            line.path.strokeColor = line.color;
        };

        for (var y = 0; y < c.height; y = y + this.GRIDSIZE) {
    ...
    checkForMidiNote: function (planet){
        if(this.checkGridCrossing(planet.position)){
            console.log("gridline crossed");
        }
