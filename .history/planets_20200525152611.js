...
simulation.prototype = {
    ...
    checkForMidiNote: function (planet) {
        var gridlineCrossedX = this.checkForGridCrossingX(planet.position);
        if (gridlineCrossedX) {
            console.log("gridline crossed :: x" + gridlineCrossedX)
            //make a midi note from the line properties
        }
        var gridlineCrossedY = this.checkForGridCrossingY(planet.position);
        if (gridlineCrossedY) {
    ...
    LASTX: 0,
    LASTY: 0,
    checkGridCrossing: function (positions) {
