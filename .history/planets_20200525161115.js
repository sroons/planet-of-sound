...
simulation.prototype = {
    ...
    checkForMidiNote: function (planet) {
        ...
        if (gridlineCrossedX) {
            console.log("gridline crossed :: x " + gridlineCrossedX)
    ...
    checkGridCrossingX: function (positions) {
        /*         var roundedX = Math.floor(positions.x);
                if (roundedX % this.GRIDSIZE == 0 && roundedX != this.LASTX) {
                    this.LASTX = roundedX;
                    return roundedX;
                }
                return false; */
        for (var x = 0; x < this.GRID.VERTICALS.length; x++) {
            var line = this.GRID.VERTICALS[x].line;
            if (line == undefined) continue;
            return line.hitTest(new Point(positions.x, positions.y));
        }
    },
    checkGridCrossingY: function (positions) {
        /*         var roundedY = Math.floor(positions.y);
                ...
                return false; */
        for (var y = 0; y < this.GRID.HORIZONTALS.length; y++) {
            var line = this.GRID.HORIZONTALS[y].line;
            if (line == undefined) continue;
            return line.line.hitTest(new Point(positions.x, positions.y))
        }
    },
    X_RANGE: [-150, 150],
    PAN_RANGE: [1, -1],
